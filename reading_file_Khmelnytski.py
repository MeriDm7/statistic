...
def file_with_total_inform(path_2):
    ...
    dict_with_total = {}
    for i in range(13):
        dict_with_total[new_lst[i * 11]] = new_lst[i * 11 + 1]  
...
def open_file(path_1):
    ...
    dict_with_total = file_with_total_inform("Total_Khmelnytskyi.csv")
    lst_with_data = lst_with_data[14:]       
    dict_by_years = {}
    for i in lst_with_data:
        dict_by_years[int(i[0])] = causes(i)
        dict_by_years[int(i[0])].update({"Total": dict_with_total[i[0]]})
    res_dict = {}
    res_dict["Khmelnytski"] = dict_by_years    
    return res_dict
...
def result():
    ...
    Return the dict.
    """
    res = open_file("Khmelnytskyi.csv")
    return res