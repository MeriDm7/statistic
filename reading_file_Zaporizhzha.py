...
def open_file(path_1):
    ...
    dict_with_total = file_with_total_inform("Total_Zaporizhzha.csv")
    lst_with_data = lst_with_data[3:] 
    dict_by_years = {}
    for i in lst_with_data:
        dict_by_years[int(i[0])] = causes(i)
        dict_by_years[int(i[0])].update({"Total": dict_with_total[i[0]]})
    res_dict = {}
    res_dict["Zaporizhzha"] = dict_by_years    
    return res_dict
