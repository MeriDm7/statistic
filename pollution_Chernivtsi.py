...
def file_with_pollution(path):
    ...
    lst_with_data = lst_with_data[272:][::2][:-8]
    res_lst = []
    for i in range(13):
        res_lst.append(lst_with_data[i * 8: i * 8 + 2])
    dict_with_years = {}
    for i in res_lst:
            dict_with_years[int(i[0][0])] = convert(i[1][0])
    res_dict = {}
    res_dict["Chernivtsi"] = dict_with_years
    return res_dict
...
def result():
    ...
    Return the dict.
    """
    res = file_with_pollution("Pollution_Chernivtsi.csv")
    return res
