...
def open_file(path_1):
    ...
    dict_with_total = file_with_total_inform("Total_Poltava.csv")
    lst_with_data = lst_with_data[30:] 
    lst_with_data = lst_with_data[::2]
    for i in range(len(lst_with_data)):
        if lst_with_data[i][0] != "":
            lst_with_data[i] = lst_with_data[i][0]
    for i in lst_with_data:
        if type(i) == list:
            lst_with_data.remove(i)
    new_lst = []
    dict_by_years = {}
    for i in range(13):
        lil_lst = []
    ...
    res_dict = {}
    res_dict["Poltava"] = dict_by_years    
    return res_dict
